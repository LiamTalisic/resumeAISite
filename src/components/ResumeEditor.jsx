...
const ResumeEditor = () => {
  ...
  const convertResumeToText = async (resumeData) => {
    ...
    if (resumeData.historyContent) {
      ...
      if (resumeData.historyContent.startsWith('data:application/pdf') || 
          resumeData.historyContent.length > 1000 && !resumeData.historyContent.includes('\\documentclass')) {
    ...
    if (resumeData.history) {
      ...
      for (const field of historyFields) {
        ...
        if (typeof value === 'string' && value.length > 100) {
          ...
          if (value.includes('Jake Ryan') || value.includes('JAKE RYAN') || 
              value.includes('Southwestern University') || value.includes('Experience') ||
              value.includes('Projects') || value.includes('Technical Skills')) {
...
Libraries: pandas, NumPy, Matplotlib`;
  ...
  const convertLatexToText = (latexContent) => {
    ...
    text = text.replace(/\\resumeSubheading\s*{([^}]*)}\s*{([^}]*)}\s*{([^}]*)}\s*{([^}]*)}/g, 
      '$1                                                              $2\n$3                                                              $4');
    
    text = text.replace(/\\resumeProjectHeading\s*{([^}]*)}\s*{([^}]*)}/g, 
      '$1                                                              $2');
...
Libraries: pandas, NumPy, Matplotlib`;
  ...
  const handleLineInput = (e, lineIndex) => {
    ...
    setResumeLines(prev => prev.map((line, index) => 
      index === lineIndex 
        ? { ...line, text: newText, isEdited: true }
        : line
    ));
  };
  ...
  const handleLineBlur = (e, lineIndex) => {
    ...
    if (!newText.trim()) {
      ...
      setResumeLines(prev => prev.map((line, index) => 
        index === lineIndex 
          ? { ...line, text: originalText, isEdited: false }
          : line
      ));
    } else {
    ...
    <div className="h-screen w-full bg-gray-50 flex flex-col">
      ...
      {isLineByLineMode && (
        <div className="px-6 py-3 bg-blue-50 border-b border-blue-200 flex items-center justify-between">
          ...
          <button
            onClick={() => setIsLineByLineMode(false)}
          ...
          >
            {resumeLines.map((line, index) => {
              ...
              return (
                <div
                  key={line.id}
                  className={`relative group ${getLineClassName(index)}`}
                ...
                >
                  ...
                  {hasRightAlignedContent ? (
                    ...
                    <div className={`min-h-[1.5rem] ${getLineStyle(line, index)} flex justify-between items-start`}>
                      ...
                      <div className="text-right font-medium text-gray-700 ml-4">
                        {text.split(/\s{10,}/)[1] || ''}
                      </div>
                    </div>
                  ) : (
